...
import { investigations } from '@/utils/mockData'
...
function Investigations() {
  ...
  const handlePatientSelect = (selectedPatient) => {
    ...
    const data = investigations.filter(i => i.patientId === selectedPatient.id)
    setPatientInvestigations(data)
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">
          {patient ? `Investigations - ${patient.name}` : 'Investigations'}
        </h1>
      ...
      {patient ? (
        ...
        <Card>
          <CardContent className="py-8">
            <p className="text-center text-muted-foreground">
              Please select a patient to view their investigations.
            </p>
          </CardContent>
        </Card>
      )}
